...
gulp.task(tsks.karma.test, ['build_test'], function (done) {
...
gulp.task('build_test', function (done) {
    sequence('compile_scripts',
        'create_config',
        'compile_test_scripts',
        tsks.ngTemplateCache.generate,
        done);
});

gulp.task('compile_test_scripts', function () {
    log('Transpiling unit test Typescript code to Javascript');

    var tsResult = gulp.src(config.karma.testTypeScriptFiles)
        .pipe($.typescript({
            target: config.karma.targetVersion,
    ...
    return merge([
        ...
        tsResult.js.pipe(gulp.dest(config.folders.devBuild + 'js'))
    ]);
});
...
gulp.task('optimize_build', () => {
    ...
    let task = gulpSrc(config.shell, 'optimize_build')
        .pipe($.plumber())
        .pipe(assets)
        .pipe(cssFilter)
        .pipe($.csso())
